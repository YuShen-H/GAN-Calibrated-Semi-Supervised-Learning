...
class HFUNet(nn.Module):
    ...
    def __init__(self, num_classes=1, input_channels=3, layer_scale_init_value=1e-6, HFConv=HFConv, block=Block,
        ...
        # ... __init__ 的所有內容保持原樣，此處省略 ...
        super().__init__()
        # ... (省略)
        self.final = nn.Conv2d(c_list[0], num_classes, kernel_size=1)
        self.apply(self._init_weights)
        
    # _init_weights 函式保持完全不變
    def _init_weights(self, m):
        # ... _init_weights 的所有內容保持原樣 ...
        pass
    ...
    def extract_features(self, x):
        ...
        # --- Bottleneck ---
        bottleneck_features = F.gelu((self.ebn6(self.encoder6(out))))
        bottleneck_features = self.ConvMixer(bottleneck_features) # (B, c_list[5], H/64, W/64)

        # 為了保持原 forward 函式可用，我們也返回跳躍連接的特徵
        skip_features = [t1, t2, t3, t4, t5]
        return bottleneck_features, skip_features

    # forward 函式現在調用 extract_features，使其更簡潔
    def forward(self, x):
        # 提取特徵
        out, skip_features = self.extract_features(x)
        t1, t2, t3, t4, t5 = skip_features

        # --- Decoder ---
        # 這裡的 MDAG 和 SC_Att_Bridge 主要是優化給分割任務的跳躍連接
        # 在回歸任務中可以考慮是否保留
        t5 = self.mdag4(x=t5)
        t4 = self.mdag3(x=t4)
        t3 = self.mdag2(x=t3)
        ...
        # ... (此處省略所有 decoder 和 torch.add 的程式碼) ...
        out5 = F.gelu(self.dbn1(self.decoder1(out)))
        out5 = torch.add(out5, t5)
...
class HFUNet_Regressor(nn.Module):
    """
    使用 HFUNet 作為骨幹網路的回歸模型。
    """
    def __init__(self, input_channels=3, delta_scale=1.0, pretrained_hfunet=None):
        super().__init__()
        
        # --- 步驟 A: 載入 HFUNet 作為骨幹網路 ---
        # 您可以在這裡傳入 HFUNet 的所有自訂參數
        # 例如 c_list, depths 等
        self.backbone = HFUNet(input_channels=input_channels, num_classes=1) # num_classes 在此無用

        # 如果您有預訓練好的分割模型權重，可以在這裡載入
        # 這會極大地加速您的訓練（遷移學習）
        if pretrained_hfunet:
            print(f"正在從 {pretrained_hfunet} 載入預訓練權重...")
            self.backbone.load_state_dict(torch.load(pretrained_hfunet), strict=False)

        # --- 步驟 B: 定義回歸頭 ---
        # 瓶頸層的輸出通道數是 c_list[5]，預設是 1024
        bottleneck_channels = self.backbone.encoder6[2].out_channels # 或者直接用 c_list[5] = 1024
        
        self.regression_head = nn.Sequential(
            # 使用全局平均池化將 (B, C, H, W) 的特徵圖轉換成 (B, C, 1, 1)
            ...
            # MLP 層，將高維特徵映射到最終的輸出
            nn.Linear(in_features=bottleneck_channels, out_features=256),
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
    ...
    def forward(self, x):
        ...
        """
        # 只取瓶頸層的特徵，忽略跳躍連接的特徵
        bottleneck_features, _ = self.backbone.extract_features(x)
        
        # 通過回歸頭得到原始的 delta 輸出
        delta_raw = self.regression_head(bottleneck_features) # shape: (B, 4)
...
if __name__ == '__main__':
    ...
    # 建立您的回歸模型
    # delta_scale 是一個超參數，您可以根據您的任務進行調整
    model = HFUNet_Regressor(input_channels=3, delta_scale=0.1).to(device)
    
    # 建立一個假的輸入圖像批次 (Batch Size=2, 3通道, 224x224)
    # HFUNet 通常在較大的尺寸上表現更好
    dummy_input = torch.randn(2, 3, 224, 224).to(device)
    
    # 進行一次前向傳播
    predicted_delta = model(dummy_input)
    
    # 打印輸出形狀，應該是 torch.Size([2, 4])
    print("模型建立成功！")
    print(f"輸入形狀: {dummy_input.shape}")
    print(f"輸出 Delta 形狀: {predicted_delta.shape}")
    print(f"輸出值範例: \n{predicted_delta}")
